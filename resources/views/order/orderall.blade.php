...
<div class="card-header border-0" style="margin:10px;">
  <div class="row align-items-center">
    <div class="col-12 text-center">
      <h1 class="mb-0" >Pesanan Yang Berkaitan</h1>
      <br>
      <form class="form" style="width: 100%"  class="text-left" action="{{url('order/all')}}" method="post">
        @csrf
        <div class="row" class="text-left">
          <div class="col">
            ...
            <div class="form-group">
              <select id="" class="form-control" name="k" style="width: 100%">
               <option @if($k == 1) selected @endif value="0">Semua Pesanan</option>
               <option value="1" @if($k == 1) selected @endif>Penanggungjawab Pada Pesanan</option>
               <option value="2" @if($k == 2) selected @endif >Kalab / Laboran Pada Pesanan</option>
               @foreach($labnya as $l)
          ...
          <div class="col">
              <h4 class="text-left">Jenis Pesanan</h4>
             <div class="form-group">
                <select id="" class="form-control" name="j" style="width: 100%">
                 <option @if($j == 0) selected @endif  value="0">Semua Pesanan</option>
                 <option @if($j == 1) selected @endif  value="1">Pesanan Aktif</option>
                 <option @if($j == 2) selected @endif  value="2">Pesanan Selesai</option>
               </select>  
             </div>
          </div>
        </div>
                ...
                <div class="row" style="margin: 20px 20px 5px 20px;">
                    <div class="col-md-9" >
                            <table>
                                <tr>
                                    <td>
                                         <h3>{{$pesanan[$i]->idorder}} </h3>
                                          <h2>{{KeranjangController::cariorang($pesanan[$i]->mahasiswa)}} </h2>
                                    </td>
                                    <td>
                                      <div class="dropdown" >
